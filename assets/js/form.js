            ...
            (input) => {
                input.onblur = () => {
                    if(!input.value.trim()){                       
                        ...
                        showSuccess(input , '');
                        checkEmail(input, "Please enter the correct email");
                        checkPassword(input, 6)
                        checkConfirmPassword(input, 'Please enter the correct password');
                    }
                }
...
function checkPassword(input, min){
    if(input.name == 'password-signup' || input.name == 'password-signin'){
        ...
        if(input.value.length < min){
            message.innerHTML = `Please enter at least ${min} characters`
        }
    }
}
